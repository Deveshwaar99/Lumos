...
const EXPENSE_CATEGORIES = [
  ...
  { id: 'seed-exp-10', name: 'Other Expense', icon: 'dots-horizontal', color: '#B0BEC5' },
];
...
const DEFAULT_ACCOUNTS = [
  { id: 'seed-acc-1', name: 'Cash', type: 'cash', icon: 'wallet', openingBalanceCents: 0 },
  { id: 'seed-acc-2', name: 'Bank Account', type: 'bank', icon: 'bank', openingBalanceCents: 0 },
  { id: 'seed-acc-3', name: 'Credit Card', type: 'card', icon: 'credit-card', openingBalanceCents: 0 },
];
...
export async function seedDemoTransactions(db: SQLiteDatabase): Promise<void> {
  ...
  const accountsResult = await db.getAllAsync<{ id: string }>('SELECT id FROM accounts');

  if (categoriesResult.length === 0 || accountsResult.length === 0) return;

  const expenseCategories = categoriesResult.filter((c) => c.type === 'expense');
  ...
  for (let i = 0; i < targetCount; i++) {
    ...
    const cat = categories[samples[i % samples.length].catIndex % categories.length];
    ...
    const split1Amount = isSplitDemo ? Math.floor(amountCents * 0.7) : amountCents;
    ...
    await db.runAsync(
      ...
      txnId, isIncome ? 'income' : 'expense', amountCents, 'USD',
      cat.id, accountId, sample.note, dateStr, null, nowStr, nowStr
    );
