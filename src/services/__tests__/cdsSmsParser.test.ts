...
console.assert(parsed1?.tradeDate === '2026-04-24', 'sample1 date should parse');
console.assert(parsed1?.movements.length === 2, 'sample1 should have two buys');
console.assert(
  parsed1?.movements[0].stockCode === 'JKH' && parsed1.movements[0].quantity === 3000,
...
console.assert(
  parsed2?.movements[1].stockCode === 'ALUM' && parsed2.movements[1].quantity === 2250,
...
console.assert(parsed4?.movements.length === 3, 'sample4 should have three buys');
...
console.assert(parsedMultiple?.movements.length === 4, 'mixed message should have 4 rows');
console.assert(
