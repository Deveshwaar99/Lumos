...
export const categoryService = {
  ...
  async getByType(type: 'income' | 'expense'): Promise<Category[]> {
    const db = await getDatabase();
    const rows = await db.getAllAsync<any>('SELECT * FROM categories WHERE type = ? ORDER BY name', type);
    return rows.map(mapRow);
  },
  ...
  async delete(id: string): Promise<{ success: boolean; message?: string }> {
    ...
    if (usage && usage.count > 0) {
      return { success: false, message: `Category is used by ${usage.count} transaction(s). Please reassign them first.` };
    }
